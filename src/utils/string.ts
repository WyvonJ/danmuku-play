...
export function replaceCnNums(str: string) {
  ...
  if (match?.length) {
    match.forEach((cnNum: any) => {
      // 阿拉伯数字
      const num = fillZero(chineseToNumber(cnNum));
      const reg = new RegExp(cnNum, 'g');
...
export function sortDirList(list: any[]): any[] {
  return list
    .sort((a, b) => {
      return replaceCnNums(a.title).localeCompare(replaceCnNums(b.title));
    })
    .map((item) => ({
      ...item,
      children: sortDirList(item.children || []),
    }));
}